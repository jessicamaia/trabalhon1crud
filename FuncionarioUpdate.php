<?php
    ...
    if ( !empty($_POST)){
		...
		if ($validacao){
                    ...
                    $sql = "UPDATE funcionario  set primeiro_nome=?, ultimo_nome=?, endereco_id=?, email=?, loja_id=?, ativo=?, usuario=?,
                    senha=?, ultima_atualizacao=? WHERE funcionario_id = ?";
                    $q = $pdo->prepare($sql);
                    $q->execute(array($primeiro_nome,$ultimo_nome, $endereco_id, $email, $loja_id,$ativo,$usuario,$senha, $ultima_atualizacao, $id));
                    Conexao::disconnect();
                    header("Location: FuncionarioIndex.php");
		}
	}
    ...
    <body>
        <div class="container">
            ...
            <div class="span10 offset1">
					...
					<div class="card-body">
                <form class="form-horizontal" action="FuncionarioUpdate.php?funcionario_id=<?php echo $id?>" method="post">
                ...
                <div class="control-group <?php echo !empty($UltimaAtualizacaoErro)?'error ': '';?>">
                    ...
                    <div class="form-actions">
                        ...
                        <a href="FuncionarioIndex.php" type="btn" class="btn btn-default">Voltar</a>
                    </div>
                </form>
							</div>
						</div>
